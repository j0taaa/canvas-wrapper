...
async function getOverdueTodoItems(courses: CanvasCourse[], apiKey: string): Promise<CanvasTodoItem[]> {
...
export async function getDashboardData(apiKey?: string): Promise<CanvasDashboardData> {
  ...
  const [profile, activeCourses, allCourses, todo, dashboardCards] = await Promise.all([
    ...
    canvasFetch<CanvasCourse[]>("/courses?per_page=100&include[]=concluded&state[]=available&state[]=completed", resolvedApiKey),
  ...
  const overdueTodo = await getOverdueTodoItems(courses, resolvedApiKey);
...
export async function submitAssignment(
  ...
  const response = await fetch(`${canvasApiBase}/courses/${courseId}/assignments/${assignmentId}/submissions`, {
    method: "POST",
    headers: {
